...
public class TileGenericPipe extends TileEntity
    ...
    protected boolean canPipeConnect_internal(TileEntity with, ForgeDirection side) {
        if (!(pipe instanceof IPipeConnectionForced)
                || !((IPipeConnectionForced) pipe).ignoreConnectionOverrides(side)) {
            if (with instanceof IPipeConnection) {
                IPipeConnection.ConnectOverride override = ((IPipeConnection) with)
                        .overridePipeConnection(pipe.transport.getPipeType(), side.getOpposite());
    ...
    public boolean hasBlockingPluggable(ForgeDirection side) {
        ...
        if (pluggable instanceof IPipeConnection) {
            ...
            if (neighborPipe != null) {
                IPipeConnection.ConnectOverride override = ((IPipeConnection) pluggable)
                        .overridePipeConnection(neighborPipe.getTile().getPipeType(), side);
                    ...
                    if (pluggable != null && pluggable instanceof GatePluggable) {
                        ...
                        if (gate == null || gate.logic != gatePluggable.getLogic()
                            ...
                            pipe.gates[i] = GateFactory.makeGate(
                                    pipe,
                                    gatePluggable.getMaterial(),
                                    gatePluggable.getLogic(),
